...
// HR:P Weapon placeholder entities.
CDebug g_Debug;
CSaber g_Saber;
CColt g_Colt;
CHenry g_Henry;
...
#endif
CGatling g_Gatling;
CDynamite g_Dynamite;
#ifdef USE_MP_WEAPONS
CHalfCatWeapon g_HalfCat;
#endif
...
BOOL CBasePlayerWeapon :: PlayEmptySound( void )
{
	...
	{
		// Shepard : Custom dryfire sound depending on the weapon we are holding.
		switch ( m_iId )
		{
		case WEAPON_DEBUG: HUD_PlaySound( "weapons/colt_cock.wav", 0.8f ); break;
		case WEAPON_COLT: HUD_PlaySound( "weapons/colt_cock.wav", 0.8f ); break;
		case WEAPON_HENRY: HUD_PlaySound( "weapons/henry_cock.wav", 0.8f ); break;
		case WEAPON_SHOTGUN: HUD_PlaySound( "weapons/shotgun_cock.wav", 0.8f ); break;
#if USE_MP_WEAPONS
		case WEAPON_BOW: break;
#endif
		case WEAPON_GATLING: HUD_PlaySound( "weapons/gatling_cock.wav", 0.8f ); break;
		default: HUD_PlaySound( "weapons/357_cock1.wav", 0.8f );
		}
		m_iPlayEmptySound = 0;
		return 0;
	}
...
void HUD_InitClientWeapons( void )
{
	...
	HUD_PrepEntity( &g_Debug	, &player );
	HUD_PrepEntity( &g_Saber	, &player );
...
#endif
	HUD_PrepEntity( &g_Gatling	, &player );
	HUD_PrepEntity( &g_Dynamite	, &player );
#ifdef USE_MP_WEAPONS
	HUD_PrepEntity( &g_HalfCat	, &player );
#endif
}
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	{
		case WEAPON_DEBUG: pWeapon = &g_Debug; break;
		case WEAPON_SABER: pWeapon = &g_Saber; break;
		case WEAPON_COLT: pWeapon = &g_Colt; break;
		case WEAPON_HENRY: pWeapon = &g_Henry; break;
		case WEAPON_SHOTGUN: pWeapon = &g_Shotgun; break;
#ifdef USE_MP_WEAPONS
		case WEAPON_BOW: pWeapon = &g_Bow; break;
#endif
		case WEAPON_GATLING: pWeapon = &g_Gatling; break;
		case WEAPON_DYNAMITE: pWeapon = &g_Dynamite; break;
#ifdef USE_MP_WEAPONS
		case WEAPON_HALFCAT: pWeapon = &g_HalfCat; break;
#endif
	}
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
		...
		pCurrent->m_iParaExtraStuff		= pfrom->iuser1;
		pCurrent->m_iParaExtraStuff2	= pfrom->iuser2;
	...
	player.ammo_debug		= (int)from->client.vuser2[2];
	player.ammo_45			= (int)from->client.vuser1[0];
	player.ammo_buckshot	= (int)from->client.vuser1[1];
	player.ammo_gatling		= (int)from->client.vuser1[2];

	player.ammo_bolts		= (int)from->client.ammo_nails; //is an int anyways...
	player.ammo_argrens		= (int)from->client.ammo_shells; 
	...
	// HR:P Weapons
	to->client.vuser2[2]				= player.ammo_debug;
	to->client.vuser1[0]				= player.ammo_45;
	to->client.vuser1[1]				= player.ammo_buckshot;
	to->client.vuser2[1]				= player.ammo_gatling;

	// HL Weapons
	to->client.ammo_nails				= player.ammo_bolts;
	to->client.ammo_shells				= player.ammo_argrens;
...
//		pto->m_flPumpTime				= pCurrent->m_flPumpTime;
		...
		pto->iuser1						= pCurrent->m_iParaExtraStuff;
		pto->iuser2						= pCurrent->m_iParaExtraStuff2;
