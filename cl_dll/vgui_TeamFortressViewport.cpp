...
// Shepard : this file contains code from Dominik "ripieces" Tugend
// His code fixes the input issues on Linux and Mac (and also improves
// it for Windows), for more information, see his GitHub repository here:
// https://github.com/ripieces/halflife
...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	m_pTeamMenu = NULL;
	m_pClassMenu = NULL;
	m_pHRPBuyMenu = NULL; // Shepard : HR:P VGUI buy menu
	m_pHRPCredits = NULL; // Shepard : HR:P VGUI credits
	m_pHRPNewGameMenu = NULL; // Shepard : HR:P VGUI new game menu
	m_pScoreBoard = NULL;
	m_pSpectatorPanel = NULL;
	m_pCurrentMenu = NULL;
	m_pCurrentCommandMenu = NULL;
	m_pSubtitles = NULL; // Shepard : PARANOIA subtitles system
	m_pDebugOverlay = NULL; // Shepard : Debugging overlay
	...
	CreateTeamMenu();
	CreateClassMenu();
	CreateHRPBuyMenu(); // Shepard : HR:P VGUI buy menu
	CreateHRPCredits(); // Shepard : HR:P VGUI credits
	CreateHRPNewGameMenu(); // Shepard : HR:P VGUI new game menu
	CreateSubtitlesMenu(); // Shepard : PARANOIA subtitles system
	CreateDebugOverlay(); // Shepard : Debugging overlay
	CreateSpectatorMenu();
	CreateScoreBoard();
	// Init command menus
	m_iNumMenus = 0;
...
void TeamFortressViewport::Initialize( void )
{
	...
	if ( m_pHRPBuyMenu )
		m_pHRPBuyMenu->Initialize(); // Shepard : HR:P VGUI buy menu
	if ( m_pHRPCredits ) // Shepard : HR:P VGUI credits
	{
		if ( m_pHRPCredits->m_bInitialized )
			m_pHRPCredits->Reset();
		else
			m_pHRPCredits->Initialize();
	}
	if ( m_pHRPNewGameMenu )
		m_pHRPNewGameMenu->Initialize(); // Shepard : HR:P VGUI new game menu
	if ( m_pSubtitles )
		m_pSubtitles->Initialize(); // Shepard : PARANOIA subtitles system
	if (m_pScoreBoard)
	{
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	if ( iTextToShow == SHOW_MOTD )
	{
		if (!m_szServerName || !m_szServerName[0])
			strcpy( cTitle, "Half-Rats: Parasomnia" );
		else
...
void TeamFortressViewport::ShowVGUIMenu( int iMenu )
{
	...
	case MENU_CLASS:
		...
		break;
	case MENU_HRP_BUY: // Shepard : HR:P VGUI buy menu
		pNewMenu = ShowHRPBuyMenu();
		break;
	case MENU_HRP_CREDITS: // Shepard : HR:P VGUI credits
		pNewMenu = ShowHRPCredits();
		break;
	case MENU_HRP_NEWGAME: // Shepard : HR:P VGUI new game menu
		pNewMenu = ShowHRPNewGameMenu();
		break;
	default:
		break;
	}
...
// Shepard : HR:P VGUI buy menu
CMenuPanel* TeamFortressViewport::ShowHRPBuyMenu( void )
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	m_pHRPBuyMenu->Reset();
	return m_pHRPBuyMenu;
}

// Shepard : HR:P VGUI credits
CMenuPanel* TeamFortressViewport::ShowHRPCredits( void )
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	m_pHRPCredits->Reset();
	return m_pHRPCredits;
}

// Shepard : HR:P VGUI new game menu
CMenuPanel* TeamFortressViewport::ShowHRPNewGameMenu( void )
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;

	m_pHRPNewGameMenu->Reset();
	return m_pHRPNewGameMenu;
}
...
// Shepard : HR:P VGUI buy menu
void TeamFortressViewport::CreateHRPBuyMenu( void )
{
	// Create the panel
	m_pHRPBuyMenu = new CHRPBuyMenuPanel( 100, false, 0, 0, ScreenWidth, ScreenHeight );
	m_pHRPBuyMenu->setParent( this );
	m_pHRPBuyMenu->setVisible( false );
}

// Shepard : HR:P VGUI credits
void TeamFortressViewport::CreateHRPCredits( void )
{
	if ( !m_pHRPCredits )
	{
		// Create the panel
		m_pHRPCredits = new CHRPCreditsPanel( 255, false, 0, 0, ScreenWidth, ScreenHeight );
		m_pHRPCredits->setParent( this );
		m_pHRPCredits->setVisible( false );
	}
	else
		m_pHRPCredits->Close();
}
...
// Shepard : HR:P VGUI new game menu
void TeamFortressViewport::CreateHRPNewGameMenu( void )
{
	// Create the panel
	m_pHRPNewGameMenu = new CHRPNewGameMenuPanel( 100, false, 0, 0, ScreenWidth, ScreenHeight );
	m_pHRPNewGameMenu->setParent( this );
	m_pHRPNewGameMenu->setVisible( false );
}

// Shepard : Debugging overlay
void TeamFortressViewport::CreateDebugOverlay()
{
	m_pDebugOverlay = new CVGUIDebugOverlay();
	m_pDebugOverlay->setParent( this );
}

// Shepard : PARANOIA subtitles system
void TeamFortressViewport::CreateSubtitlesMenu( void )
{
	m_pSubtitles = new CSubtitles();
	m_pSubtitles->setParent( this );
	m_pSubtitles->setVisible( false );
}
...
void TeamFortressViewport::paintBackground()
{
	...
	if ( m_pDebugOverlay )
	{
		bool bShowOverlay = gEngfuncs.pfnGetCvarFloat( "cl_hrp_debug_overlay" ) > 0.0f;
		if ( bShowOverlay )
			m_pDebugOverlay->Update();

		m_pDebugOverlay->setVisible( bShowOverlay );
	}
}
...
int TeamFortressViewport::MsgFunc_VGUIMenu(const char *pszName, int iSize, void *pbuf )
{
	...
	int iMenu = READ_BYTE();
	int iBuyMenuThroughUseKey = READ_BYTE();

	if ( iMenu == 10 && iBuyMenuThroughUseKey == 1 && gEngfuncs.pfnGetCvarFloat( "cl_hrp_buy_menu_use_key" ) <= 0.0f )
	{
		HUD_PlaySound( "common/wpn_denyselect.wav", 1.0f );
		return 1;
	}
