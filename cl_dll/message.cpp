...
void SubtitlesMessageAdd( client_textmessage_t *tempMessage );
...
void CHudMessage::MessageAdd( const char *pName, float time )
{
	...
	for ( i = 0; i < maxHUDMessages; i++ )
	{
		if ( !m_pMessages[i] )
		{
			...
			else if ( tempMessage->effect == 3 ) // Shepard : PARANOIA's subtitles system
			{
				SubtitlesMessageAdd( tempMessage );
				return;
			}
