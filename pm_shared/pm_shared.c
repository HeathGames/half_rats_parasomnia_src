...
==============
*/
// Shepard - Slightly modified for the Teddy freeze attack
void PM_CheckParamters( void )
{
	float fRatio;
	float spd;
	float maxspeed;
	qboolean freeze = false;
	vec3_t	v_angle;
	...
	if ( ( spd != 0.0 ) &&
		 ( spd > pmove->maxspeed ) )
	{
		freeze = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "freeze" ) ) == 1 ? true : false;
		if ( freeze )
			fRatio = 0.0f;
		else
			fRatio = pmove->maxspeed / spd;
