...
class CWoman : public CTalkMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_woman, CWoman );

void CWoman::Spawn( void )
{
	Precache();
	if ( pev->model )
		SET_MODEL( ENT( pev ), STRING( pev->model ) ); // LRC
	else
		SET_MODEL( ENT( pev ), "models/woman.mdl" );
...
void CWoman::Precache( void )
{
	if ( pev->model ) // LRC
		...
		PRECACHE_MODEL( "models/woman.mdl" );

	// Every new woman must call this, otherwise when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	CTalkMonster::Precache();
}

void CWoman::SetYawSpeed( void )
{
	int iYawSpeed = 90;
	switch ( m_Activity )
	{
...
int CWoman::Classify( void )
{
	return CLASS_HUMAN_PASSIVE;
}

void CWoman::Killed( entvars_t *pevAttacker, int iGib )
{
	SetUse( NULL );
	CTalkMonster::Killed( pevAttacker, iGib );
}

void CWoman::DeclineFollowing( void )
{
	Talk( 10.0f );
	m_hTalkTarget = m_hEnemy;
	PlaySentence( "WO_POK", 0.8f, 1.0f, ATTN_NORM );
}

void CWoman::TalkInit( void )
{
	CTalkMonster::TalkInit();

	// Woman will try to talk to friends in this order
	m_szFriends[0] = "monster_woman";
	...
	m_szGrp[TLK_PQUESTION] =	"WO_PQUESTION";
