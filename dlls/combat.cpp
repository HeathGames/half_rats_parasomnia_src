...
{
	BOOL fade = CVAR_GET_FLOAT("violence_hgibs") == 0;
...
//=========================================================
CBaseEntity* CBaseMonster :: CheckTraceHullAttack( float flDist, int iDamage, int iDmgType, BOOL bCheckFriendlyFire )
{
	TraceResult tr;
	...
	if ( tr.pHit )
	{
		...
		// Shepard : Check if a friendly NPC (like Half-Cat) is harming another friend or the player
		// if it's the case, remove the damage
		if ( bCheckFriendlyFire && pEntity->pev->flags & FL_MONSTER )
		{
			if ( IRelationship( this ) == CLASS_PLAYER_ALLY &&
				(IRelationship( pEntity ) == CLASS_PLAYER_ALLY || IRelationship( pEntity ) == CLASS_PLAYER) )
				iDamage = 0;
		}
...
void CBaseEntity::FireBullets(ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker )
{
	...
	// Osiris : muzzleflash dynamic light
	...
	// Shepard : Play "tailing" if we are outside
	UTIL_TraceLine( pev->origin, pev->origin + gpGlobals->v_up * 8192.0f, dont_ignore_monsters, ENT( pev ), &tr );
	if ( UTIL_PointContents( tr.vecEndPos ) == CONTENTS_SKY )
	{
		switch ( RANDOM_LONG( 0, 3 ) )
		{
		case 0: UTIL_EmitAmbientSound( ENT( pev ), pev->origin, "weapons/tail1.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM ); break;
		case 1: UTIL_EmitAmbientSound( ENT( pev ), pev->origin, "weapons/tail2.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM ); break;
		case 2: UTIL_EmitAmbientSound( ENT( pev ), pev->origin, "weapons/tail3.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM ); break;
		case 3: UTIL_EmitAmbientSound( ENT( pev ), pev->origin, "weapons/tail4.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM ); break;
		}
	}
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			default:
			case BULLET_MONSTER_COLT:
				pEntity->TraceAttack( pevAttacker, gSkillData.monDmgColt, vecDir, &tr, DMG_BULLET );
				TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
				DecalGunshot( &tr, iBulletType );
				break;
			case BULLET_MONSTER_HENRY:
				pEntity->TraceAttack( pevAttacker, gSkillData.monDmgHenry, vecDir, &tr, DMG_BULLET );
				TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
				DecalGunshot( &tr, iBulletType );
				break;
			case BULLET_MONSTER_12MM:		
...
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			// HR:P damage boost
			float flDmgBoost = 0.0f;
			CBaseEntity *pPlayerEntity = CBaseEntity::Instance( pevAttacker );
			if ( pPlayerEntity && pPlayerEntity->IsPlayer() )
				flDmgBoost = ((CBasePlayer *)pPlayerEntity)->GetDamageBoost();

			if ( iDamage )
			{
				pEntity->TraceAttack(pevAttacker, (iDamage + (int)flDmgBoost), vecDir, &tr, DMG_BULLET | ((iDamage > 16) ? DMG_ALWAYSGIB : DMG_NEVERGIB) );
			...
			{
			default:
			case BULLET_PLAYER_COLT: pEntity->TraceAttack( pevAttacker, (gSkillData.plrDmgColt + (int)flDmgBoost), vecDir, &tr, DMG_BULLET ); break;
			case BULLET_PLAYER_HENRY: pEntity->TraceAttack( pevAttacker, (gSkillData.plrDmgHenry + (int)flDmgBoost), vecDir, &tr, DMG_BULLET ); break;
			case BULLET_PLAYER_BUCKSHOT: pEntity->TraceAttack( pevAttacker, (gSkillData.plrDmgBuckshot + (int)flDmgBoost), vecDir, &tr, DMG_BULLET ); break;
			case BULLET_PLAYER_GATLING: pEntity->TraceAttack( pevAttacker, (gSkillData.plrDmgGatling + (int)flDmgBoost), vecDir, &tr, DMG_BULLET ); break;
			case BULLET_NONE: // FIX 
				pEntity->TraceAttack(pevAttacker, (50 + (int)flDmgBoost), vecDir, &tr, DMG_CLUB);
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				// only decal glass
				if ( !FNullEnt(tr.pHit) && VARS(tr.pHit)->rendermode != 0)
				{
