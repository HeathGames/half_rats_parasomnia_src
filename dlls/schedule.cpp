...
case TASK_GET_PATH_TO_BESTSCENT:
	...
	case TASK_SOUND_IDLE:
		{
			// Shepard - Play idle sounds only on "Easy" difficulty and don't spam the sounds
			if ( gpGlobals->time > m_flNextIdleSound && g_iSkillLevel == 1 )
			{
				IdleSound();
				m_flNextIdleSound = gpGlobals->time + RANDOM_FLOAT( 15.0f, 30.0f );
			}
			TaskComplete();
			break;
		}
	...
	case TASK_CLEAR_FAIL_SCHEDULE:
		...
		TaskComplete();
		break;
	// Shepard : Move to the AI marker placed by the player
	case TASK_GET_PATH_TO_AI_MARKER:
		{
		CBaseEntity *pAIMarker = UTIL_FindEntityByClassname( NULL, "ai_marker" );
		if ( pAIMarker && MoveToLocation( m_movementActivity, 2, pAIMarker->Center() ) )
			TaskComplete();
		else
			TaskFail();
		}
