...
#include "netadr.h"
#include "pm_shared.h"
#include "hrp_entity_ai_marker.h" // Shepard
#include "items.h" // Shepard
...
void LinkUserMessagesHRP( void ); // Shepard

extern int gmsgFlashlight; // Shepard
extern int gmsgVGUIMenu; // Shepard

// Shepard
#define PRICE_ITEM_COCAINE 500
#define PRICE_ITEM_DRYMEAT 45
#define PRICE_ITEM_MEATPIE 100
#define PRICE_ITEM_WHISKEY 50
#define PRICE_ITEM_WHALEOIL 70
#define PRICE_AMMO_45 40
#define PRICE_AMMO_BUCKSHOT 100
#define QUANTITY_AMMO_45 50
#define QUANTITY_AMMO_BUCKSHOT 24
...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq( pcmd, "buy" ) )
	{
		...
		// Don't do/show anything if we are not in the buy zone
		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		if ( !pPlayer->IsInBuyZone() )
			return;

		if ( CMD_ARGC() < 2 ) // No ID, show the first menu
		{
			if ( CVAR_GET_FLOAT( "cl_hrp_use_vgui_buy_menu" ) == 1.0f )
			{
				...
				pPlayer->ShowMenu( 0x7F, 0, FALSE, "#Buy_Menu1", 1 );

			return;
		}
		else // An ID has been passed
		{
			switch ( atoi( CMD_ARGV( 1 ) ) ) // Determine the item ID and buy it
			{
			case 1:
				if ( pPlayer->m_rgItems[ITEM_COCAINE] >= 9 )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_CocaineFull" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_ITEM_COCAINE )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				else
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_CocaineOk" );
					pPlayer->m_rgItems[ITEM_COCAINE]++;
					pPlayer->m_iMoney -= PRICE_ITEM_COCAINE;
					EMIT_SOUND( ENT( pPlayer->pev ), CHAN_VOICE, "items/buy.wav", 1.0f, ATTN_NORM );
					return;
				}
				break;
			case 2:
				if ( pPlayer->m_rgItems[ITEM_DRYMEAT] >= 9 )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_DrymeatFull" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_ITEM_DRYMEAT )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				...
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_DrymeatOk" );
					pPlayer->m_rgItems[ITEM_DRYMEAT]++;
					pPlayer->m_iMoney -= PRICE_ITEM_DRYMEAT;
					EMIT_SOUND( ENT( pPlayer->pev ), CHAN_VOICE, "items/buy.wav", 1.0f, ATTN_NORM );
					return;
				}
				break;
			case 3:
				if ( pPlayer->m_rgItems[ITEM_MEATPIE] >= 9 )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_MeatpieFull" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_ITEM_MEATPIE )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				...
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_MeatpieOk" );
					pPlayer->m_rgItems[ITEM_MEATPIE]++;
					pPlayer->m_iMoney -= PRICE_ITEM_MEATPIE;
					EMIT_SOUND( ENT( pPlayer->pev ), CHAN_VOICE, "items/buy.wav", 1.0f, ATTN_NORM );
					return;
				}
				break;
			case 4:
				if ( pPlayer->m_rgItems[ITEM_WHISKEY] >= 9 )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_WhiskeyFull" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_ITEM_WHISKEY )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				...
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_WhiskeyOk" );
					pPlayer->m_rgItems[ITEM_WHISKEY]++;
					pPlayer->m_iMoney -= PRICE_ITEM_WHISKEY;
					EMIT_SOUND( ENT( pPlayer->pev ), CHAN_VOICE, "items/buy.wav", 1.0f, ATTN_NORM );
					return;
				}
				break;
			case 5:
				if ( pPlayer->m_rgItems[ITEM_WHALEOIL] >= 9 )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_WhaleOilFull" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_ITEM_WHALEOIL )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				...
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_WhaleOilOk" );
					pPlayer->m_rgItems[ITEM_WHALEOIL]++;
					pPlayer->m_iMoney -= PRICE_ITEM_WHALEOIL;
					EMIT_SOUND( ENT( pPlayer->pev ), CHAN_VOICE, "items/buy.wav", 1.0f, ATTN_NORM );
					return;
				}
				break;
			case 6:
				if ( pPlayer->m_rgAmmo[pPlayer->GetAmmoIndex( "45" )] >= _45_MAX_CARRY )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_Ammo45Full" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_AMMO_45 )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				...
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_Ammo45Ok" );
					pPlayer->GiveAmmo( QUANTITY_AMMO_45, "45", _45_MAX_CARRY );
					pPlayer->m_iMoney -= PRICE_AMMO_45;
					EMIT_SOUND( ENT( pPlayer->pev ), CHAN_VOICE, "items/buy.wav", 1.0f, ATTN_NORM );
					return;
				}
				break;
			case 7:
				if ( pPlayer->m_rgAmmo[pPlayer->GetAmmoIndex( "buckshot" )] >= BUCKSHOT_MAX_CARRY )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_AmmoBuckshotFull" );
					return;
				}
				else if ( pPlayer->m_iMoney < PRICE_AMMO_BUCKSHOT )
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_NotEnoughCash" );
					return;
				}
				...
				{
					ClientPrint( pPlayer->pev, HUD_PRINTTALK, "#Buy_AmmoBuckshotOk" );
					pPlayer->GiveAmmo( QUANTITY_AMMO_BUCKSHOT, "buckshot", BUCKSHOT_MAX_CARRY );
				...
				break;
			default:
				ALERT( at_console, "[SHOP] Invalid item ID\n" );
			}
		}
	}
	// Shepard : This might cause a conflict and issue with "menuselect" from CHalfLifeTeamplay if HR:P goes MP
	else if ( FStrEq( pcmd, "menuselect" ) )
	{
		...
		int iSlot = atoi( CMD_ARGV( 1 ) );
		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		switch ( pPlayer->m_iMenuID )
		{
		case 1: // #Buy_Menu1
			if ( iSlot < 5 )
			{
				char cCommand[64];
				sprintf( cCommand, "buy %d\n", iSlot );
				CLIENT_COMMAND( pPlayer->edict(), cCommand );
			}
			else if ( iSlot == 5 )
				pPlayer->ShowMenu( 0x7F, 0, FALSE, "#Buy_Menu2", 2 );
			else
				pPlayer->ShowMenu( 0x00, 1, FALSE, "", 0 );
			break;
		case 2: // #Buy_Menu2
			if ( iSlot < 4 )
			{
	...
	else if ( FStrEq( pcmd, "drymeat" ) )
	{
		if ( g_pGameRules->IsMultiplayer() )
			return;

		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		pPlayer->m_iSelectedItem = ITEM_DRYMEAT;
		pPlayer->UseItem();
	}
	else if ( FStrEq( pcmd, "meatpie" ) )
	{
		if ( g_pGameRules->IsMultiplayer() )
			return;

		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		pPlayer->m_iSelectedItem = ITEM_MEATPIE;
		pPlayer->UseItem();
	}
	else if ( FStrEq( pcmd, "whiskey" ) )
	{
		if ( g_pGameRules->IsMultiplayer() )
			return;

		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		pPlayer->m_iSelectedItem = ITEM_WHISKEY;
		pPlayer->UseItem();
	}
	else if ( FStrEq( pcmd, "whale_oil" ) )
	{
		if ( g_pGameRules->IsMultiplayer() )
			return;

		CBasePlayer *pPlayer = GetClassPtr( (CBasePlayer *)pev );
		pPlayer->m_iSelectedItem = ITEM_WHALEOIL;
		pPlayer->UseItem();
	}
	else if ( FStrEq( pcmd, "switch_item" ) )
	{
		if ( g_pGameRules->IsMultiplayer() )
			return;

		GetClassPtr( (CBasePlayer *)pev )->SwitchItem();
	}
	else if ( FStrEq( pcmd, "use_item" ) )
	{
		if ( g_pGameRules->IsMultiplayer() )
			return;


		GetClassPtr( (CBasePlayer *)pev )->UseItem();
	}
	// HR:P AI Marker
	else if ( FStrEq( pcmd, "ai_marker" ) )
	{
		...
		// We already have a marker, remove it
		if ( pPlayer->m_pAIMarker )
		{
			//ClientPrint( pPlayer->pev, HUD_PRINTCENTER, "#AI_Marker_Removed" );
			UTIL_Remove( pPlayer->m_pAIMarker );
			pPlayer->m_pAIMarker = NULL;
			return;
		}

		// The following is a copy/paste from the dynamite toss code
		Vector vecAngThrow = pPlayer->pev->v_angle + pPlayer->pev->punchangle;
		if ( vecAngThrow.x < 0.0f )
			vecAngThrow.x = -10.0f + vecAngThrow.x * ((90.0f - 10.0f) / 90.0f);
		else
			vecAngThrow.x = -10.0f + vecAngThrow.x * ((90.0f + 10.0f) / 90.0f);
		...
		Vector vecThrow = gpGlobals->v_forward * 274.0f + pPlayer->pev->velocity;

		CAIMarker *pAIMarker = GetClassPtr( (CAIMarker *)NULL );
		pAIMarker->Spawn();
		UTIL_SetOrigin( pAIMarker->pev, vecSrc );
		pAIMarker->pev->velocity = vecThrow;
		pAIMarker->pev->angles = UTIL_VecToAngles( pAIMarker->pev->velocity );
		pAIMarker->pev->owner = ENT( pPlayer->pev );
		pAIMarker->pev->gravity = 0.5f;
		pAIMarker->pev->friction = 0.8f;
		pPlayer->m_pAIMarker = pAIMarker;

		// Determine where the marker should spawn
		/*TraceResult pTr;
		Vector vecStart = pev->origin + pev->view_ofs;
		Vector vecEnd = vecStart + gpGlobals->v_forward * 1024.0f;
		UTIL_TraceLine( vecStart, vecEnd, dont_ignore_monsters, pPlayer->edict(), &pTr );
		Vector vecOrigin = pTr.vecEndPos;
		if ( pTr.flFraction != -1.0f )
		{
			CBaseEntity *pHit = CBaseEntity::Instance( pTr.pHit );
			if ( pHit->IsBSPModel() )
				vecOrigin = vecOrigin + (gpGlobals->v_forward * -64.0f); // To make sure the marker isn't stuck
			else
				ClientPrint( pPlayer->pev, HUD_PRINTCENTER, "#AI_Marker_Invalid_Pos" );
		}

		// Spawn the marker
		pPlayer->m_pAIMarker = CBaseEntity::Create( "ai_marker", vecOrigin, Vector( 0.0f, 0.0f, 0.0f ), pPlayer->edict() );
		//ClientPrint( pPlayer->pev, HUD_PRINTCENTER, "#AI_Marker_Placed" );*/
	}
...
void ServerActivate( edict_t *pEdictList, int edictCount, int clientMax )
{
	...
	LinkUserMessages();
	LinkUserMessagesHRP(); // Shepard
}

// LRC - a cached version of gpGlobals->frametime. The engine sets frametime to 0 if the player is frozen... so we just cache it in prethink,
// allowing it to be restored later and used by CheckDesiredList.
float gflPreviousGlobalsFrameTime = 0.0f;
...
void PlayerPreThink( edict_t *pEntity )
{
	...
	gflPreviousGlobalsFrameTime = gpGlobals->time; // LRC
}
...
void PlayerPostThink( edict_t *pEntity )
{
	...
	// LRC - use the old frametime, even if the engine has reset it
	gpGlobals->frametime = gflPreviousGlobalsFrameTime;
}
...
// player gib sounds
	...
	// HR:P items
	PRECACHE_SOUND( "items/buy.wav" );
	PRECACHE_SOUND( "items/cocaine.wav" );
	PRECACHE_SOUND( "items/drunk.wav" );
	PRECACHE_SOUND( "items/drunk2.wav" );
	PRECACHE_SOUND( "items/drunk3.wav" );
	PRECACHE_SOUND( "items/drunk4.wav" );
	PRECACHE_SOUND( "items/drunk5.wav" );
	PRECACHE_SOUND( "items/food.wav" );
	PRECACHE_SOUND( "items/food2.wav" );
	PRECACHE_SOUND( "items/whaleoil.wav" );
	PRECACHE_SOUND( "items/whiskey.wav" );

	// Shepard : Death/drown sounds
	...
	PRECACHE_SOUND( "player/drown3.wav" );
	PRECACHE_SOUND( "player/drown4.wav" );
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					if ( II.iId >= 0 && II.iId < 32 )
					{
						...
						item->iuser1					= gun->m_iParaExtraStuff;
						item->iuser2					= gun->m_iParaExtraStuff2;
...
void UpdateClientData ( const edict_t *ent, int sendweapons, struct clientdata_s *cd )
{
	...
	if ( sendweapons )
	{
		if ( pl )
		{
			...
			cd->vuser2.z		= pl->ammo_debug;
			cd->vuser1.x		= pl->ammo_45;
			cd->vuser1.y		= pl->ammo_buckshot;
			cd->vuser1.z		= pl->ammo_gatling;

			cd->ammo_shells		= pl->ammo_argrens;
