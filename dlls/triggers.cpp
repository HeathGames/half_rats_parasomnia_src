...
// Shepard : This is now the SoHL version
...
// LRC - Was CBaseToggle, now CBaseEntity
...
public:
	...
	int m_iMode;
	int m_iszThreadName;
	int m_iszLocusThread;
	string_t m_sMaster;

private:
	USE_TYPE m_iTriggerType; // LRC
...
TYPEDESCRIPTION	CMultiManager::m_SaveData[] = 
{
	...
	// LRC
	DEFINE_FIELD( CMultiManager, m_flMaxWait, FIELD_TIME ),
	DEFINE_FIELD( CMultiManager, m_flWait, FIELD_TIME ),
	DEFINE_FIELD( CMultiManager, m_hActivator, FIELD_EHANDLE ),
	DEFINE_FIELD( CMultiManager, m_iMode, FIELD_INTEGER ),
	DEFINE_FIELD( CMultiManager, m_iState, FIELD_INTEGER ),
	DEFINE_FIELD( CMultiManager, m_iszLocusThread, FIELD_STRING ),
	DEFINE_FIELD( CMultiManager, m_iszThreadName, FIELD_STRING ),
	DEFINE_FIELD( CMultiManager, m_iTriggerType, FIELD_INTEGER ), 
	DEFINE_FIELD( CMultiManager, m_sMaster, FIELD_STRING )
};
...
void CMultiManager :: KeyValue( KeyValueData *pkvd )
{
	...
	else if ( FStrEq( pkvd->szKeyName, "triggerstate" ) )
	{
		...
		{
			case 3:  pev->spawnflags |= SF_MULTIMAN_SAMETRIG;   break;
			case 1:	 m_iTriggerType = USE_ON;     break;
			case 2:	 m_iTriggerType = USE_OFF;    break;
			default: m_iTriggerType = USE_TOGGLE; break;
		}
...
void CMultiManager :: ManagerThink ( void )
{
	...
	if ( m_iMode )
	{
		if ( pev->spawnflags & SF_MULTIMAN_LOOP )
		{
			...
			pev->nextthink = gpGlobals->time + m_startTime;
			m_startTime = gpGlobals->time + m_startTime;
		...
		if ( m_iMode == MM_MODE_CHOOSE )
		{
			...
			if ( total == 0.0f )
			{
				...
				FireTargets( sTarg, m_hActivator, this, m_iTriggerType, 0.0f );
			}
			else
			{
				float chosen = RANDOM_FLOAT( 0.0f, total );
				float curpos = 0.0f;
				for ( i = 0; i < m_cTargets; i++ )
				{
					...
					if ( curpos >= chosen )
					{
						FireTargets( STRING( m_iTargetName[i] ), m_hActivator, this, m_iTriggerType, 0.0f );
						break;
					}
				}
			}
		}
		else if ( m_iMode == MM_MODE_PERCENT )
		{
			for ( i = 0; i < m_cTargets; i++ )
			{
...
// ok, so m_iMode is 0; we're doing normal time-based stuff.
	...
	{
		FireTargets( STRING( m_iTargetName[index] ), m_hActivator, this, m_iTriggerType, 0.0f );
		index++;
	}
}
...
CMultiManager *CMultiManager::Clone( void )
{
	...
	// LRC
	if ( m_iszThreadName )
		pMulti->pev->targetname = m_iszThreadName;
	pMulti->m_iTriggerType = m_iTriggerType;
...
void CMultiManager :: ManagerUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( ShouldClone() )
	{
		...
		// LRC
		if ( m_iszLocusThread )
			FireTargets( STRING( m_iszLocusThread ), pClone, this, USE_TOGGLE, 0.0f );
	...
	if ( pev->spawnflags & SF_MULTIMAN_SAMETRIG )
		m_iTriggerType = useType;
	...
	{
		SetThink( &CMultiManager::ManagerThink );
		pev->nextthink = gpGlobals->time + timeOffset;
	}
	else
	{
		SetThink( &CMultiManager::ManagerThink );
...
// Shepard : CBaseTrigger declaration moved to "cbase.h" to
// allow CBaseTrigger entities to be created in other files
// than "triggers.cpp"
...
void CChangeLevel :: ChangeLevelNow( CBaseEntity *pActivator )
{
	...
	// Shepard : Remove the AI marker of the player has one
	CBasePlayer *plr = (CBasePlayer *)pPlayer;
	if ( plr )
	{
		plr->m_bNewGamePlusTriggersEnabled = FALSE;
		if ( plr->m_pAIMarker )
		{
			//ClientPrint( pPlayer->pev, HUD_PRINTCENTER, "#AI_Marker_Removed" );
			UTIL_Remove( plr->m_pAIMarker );
			plr->m_pAIMarker = NULL;
		}
	}
...
void CTriggerCamera::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// Shepard : shut down lantern if we are using a camera
	...
	if ( pPlayer )
	{
		...
		ALERT( at_console, "[HR:P] trigger_camera user is not a player or the player cast failed!\n" );
