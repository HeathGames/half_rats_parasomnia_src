...
//	DEFINE_FIELD( CBaseMonster, m_afSoundTypes, FIELD_INTEGER ),
	...
	// Shepard - Better idle sounds handling
	DEFINE_FIELD( CBaseMonster, m_flNextIdleSound, FIELD_TIME )
};
...
// Shepard : Thanks ts2do from The Wavelength for this
#include "pm_materials.h"
extern "C" char PM_FindTextureType( char *name );
void NPC_Step( CBaseMonster *pMonster, BOOL bLeftStep )
{
	TraceResult pTr;
	UTIL_TraceLine( pMonster->pev->origin + Vector( 0.0f, 0.0f, 8.0f ), pMonster->pev->origin - Vector( 0.0f, 0.0f, 16.0f ), dont_ignore_monsters, ENT( pMonster->pev ), &pTr );
	CBaseEntity *pEntity = CBaseEntity::Instance( pTr.pHit );

	char cTextureType = 0;
	const char *cTextureName = TRACE_TEXTURE( ENT( pEntity ? pEntity->pev : 0 ), pMonster->pev->origin + Vector( 0.0f, 0.0f, 8.0f ), pMonster->pev->origin - Vector( 0.0f, 0.0f, 16.0f ) );

	if ( cTextureName )
	{
		if ( *cTextureName == '-' || *cTextureName == '+' )
			cTextureName += 2;

		if ( *cTextureName == '{' || *cTextureName == '!' || *cTextureName == '~' || *cTextureName == ' ' )
			cTextureName++;

		char cBuffer[21];
		strcpy( cBuffer, cTextureName );
		cBuffer[CBTEXTURENAMEMAX - 1] = 0;
		cTextureType = PM_FindTextureType( (char *)cTextureName );
	}

	float flVol = pMonster->m_Activity == ACT_WALK ? 0.25f : 0.5f;
	char cSound[32] = "player/pl_";
	switch ( cTextureType )
	{
	case CHAR_TEX_DIRT: strcat( cSound, "dirt" ); break;
	case CHAR_TEX_VENT: strcat( cSound, "duct" ); break;
	case CHAR_TEX_GRATE: strcat( cSound, "grate" ); break;
	case CHAR_TEX_TILE: strcat( cSound, "tile" ); break;
	case CHAR_TEX_SLOSH: strcat( cSound, "slosh" ); break;
	default: strcat( cSound, "step" );
	}

	if ( bLeftStep )
		strcat( cSound, RANDOM_LONG( 0, 1 ) ? "3" : "1" );
	else
		strcat( cSound, RANDOM_LONG( 0, 1 ) ? "2" : "4" );

	strcat( cSound, ".wav" );
	UTIL_EmitAmbientSound( ENT( pMonster->pev ), pTr.vecEndPos, cSound, flVol, ATTN_NORM, 0, PITCH_NORM );
}
...
void CBaseMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCRIPT_EVENT_SOUND:			// Play a named wave file
		// Shepard : Dynamic NPC step
		if ( FStrEq( pEvent->options, "common/npc_step1.wav" ) || FStrEq( pEvent->options, "common/npc_step3.wav" ) )
			NPC_Step( this, TRUE );
		else if ( FStrEq( pEvent->options, "common/npc_step2.wav" ) || FStrEq( pEvent->options, "common/npc_step4.wav" ) )
			NPC_Step( this, FALSE );
		else
...
{
	// Shepard - Don't fade friendly NPCs and bosses
	if ( FClassnameIs( pev, "monster_bernard" ) || FClassnameIs( pev, "monster_caleb" ) || FClassnameIs( pev, "monster_huntsman" ) || Classify() == CLASS_HUMAN_PASSIVE )
		return FALSE;
