...
#include "scripted.h"
#include "defaultai.h"
#include "player.h" // LRC
#include "talkmonster.h" // Shepard : To delay NPCs sentences when HR speak
...
void CScriptedSentence :: FindThink( void )
{
	if ( !m_iszEntity ) // LRC - no target monster given, speak through HEV
	{
		...
		if ( pPlayer )
		{
			...
			pev->nextthink = gpGlobals->time + m_flDuration;
		}
		else
			ALERT( at_console, "[HR:P] Can\'t play \"%s\" because there is no \"player\" entity!\n", FStringNull( pev->targetname ) ? "scripted_sentence" : STRING( pev->targetname ) );

		CTalkMonster::g_talkWaitTime = gpGlobals->time + m_flDuration;
		return;
	}
