...
CBaseEntity *UTIL_FindEntityByTargetname( CBaseEntity *pStartEntity, const char *szName, CBaseEntity *pActivator ) // LRC
{
...
// Shepard : PARANOIA subtitles system
void UTIL_ShowMessagePVS( const char *pString, const Vector &vecOrg )
{
	int i;
	for ( i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		{
			float flDist = (pPlayer->pev->origin - vecOrg).Length();
			if ( flDist > 4096.0f )
				continue;

			MESSAGE_BEGIN( MSG_PVS, gmsgHudText, vecOrg );
				WRITE_STRING( pString );
			MESSAGE_END();
		}
	}
...
// Shepard : Moved from barney.cpp (and removed static)
