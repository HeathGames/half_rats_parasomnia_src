...
extern void LinkUserMessagesHRP( void ); // Shepard
...
#define	FLASH_DRAIN_TIME	 2.4 // 100 units/6 minutes
...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	// HR:P player stuff
	DEFINE_FIELD( CBasePlayer, m_bCocaineFadeOut, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBasePlayer, m_bNewGamePlusTriggersEnabled, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBasePlayer, m_flCinematicFadeTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flCocaineTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flCocaineSwitchTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flDrymeatTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flSpeed, FIELD_FLOAT ),
	DEFINE_FIELD( CBasePlayer, m_flTeddyUnfreezeTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flWhiskeyTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_flWhiskeyNextDrunkTime, FIELD_TIME ),
	DEFINE_FIELD( CBasePlayer, m_iGameEnd, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iMoney, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iSelectedItem, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_iWhiskeyCount, FIELD_INTEGER ),
	DEFINE_FIELD( CBasePlayer, m_pAIMarker, FIELD_EHANDLE )
};	
...
void CBasePlayer :: DeathSound( void )
{
	...
	if ( pev->waterlevel == 3 )
	{
		...
		{
		case 0: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/drown1.wav", 1, ATTN_NONE ); break;
		case 1: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/drown2.wav", 1, ATTN_NONE ); break;
		case 2: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/drown3.wav", 1, ATTN_NONE ); break;
		case 3: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/drown4.wav", 1, ATTN_NONE ); break;
		}
		return;
	}
	...
	{
	case 0: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die1.wav", 1, ATTN_NONE ); break;
	case 1: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die2.wav", 1, ATTN_NONE ); break;
	case 2: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die3.wav", 1, ATTN_NONE ); break;
	case 3: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die4.wav", 1, ATTN_NONE ); break;
	case 4: EMIT_SOUND( ENT( pev ), CHAN_VOICE, "player/die5.wav", 1, ATTN_NONE ); break;
	}
}
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// Shepard : Deny any damage if the player is frozen
	if ( pev->flags & FL_FROZEN )
		return 0;

	// Shepard : Are we blocking a club or slash damage with the saber ?
	if ( ((bitsDamageType & DMG_SLASH) || (bitsDamageType & DMG_CLUB)) && IsBlocking( pAttacker ) )
		return 0;

	// Shepard : Under cocaine effect (not crush damage)
	if ( m_flCocaineTime > gpGlobals->time && !(bitsDamageType & DMG_CRUSH) )
		return 0;

	// Shepard : Under cocaine effect (fall damage)
	if ( m_flCocaineTime > gpGlobals->time && (bitsDamageType & DMG_FALL) )
		flDamage /= 2.0f;

	// Shepard : Mutator "Fragile"
	if ( m_iGameEnd == 1 && CVAR_GET_FLOAT( "sv_hrp_bonus_fragile" ) > 0.0f && ((bitsDamageType & DMG_BULLET) ||
		(bitsDamageType & DMG_CLUB) || (bitsDamageType & DMG_ACID) || (bitsDamageType & DMG_SLASH)) )
		flDamage = 999.0f;
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	// Shepard : Remove the AI marker
	if ( m_pAIMarker )
	{
		UTIL_Remove( m_pAIMarker );
		m_pAIMarker = NULL;
	}

	// Shepard : Stop the music
...
{
	ammo_debug = AmmoInventory( GetAmmoIndex( "debug" ) );
	ammo_45 = AmmoInventory( GetAmmoIndex( "45" ) );
	ammo_buckshot = AmmoInventory( GetAmmoIndex( "buckshot" ) );
#ifdef USE_MP_WEAPONS
	ammo_arrow = AmmoInventory( GetAmmoIndex( "arrow" ) );
#endif
	ammo_gatling = AmmoInventory( GetAmmoIndex( "gatling" ) );
...
//			ALERT( at_console, "%s : %f\n", STRING( pObject->pev->classname ), flDot );
	...
	else
	{
		if ( m_afButtonPressed & IN_USE )
		{
			if ( IsInBuyZone() )
			{
				MESSAGE_BEGIN( MSG_ONE, gmsgVGUIMenu, NULL, ENT( pev ) );
					WRITE_BYTE( 10 );
					WRITE_BYTE( 1 );
				MESSAGE_END();
			}
...
// many features in this function use v_forward, so makevectors now.
	...
	{
		// Shepard : Reduce velocity if we are holding the gatling gun
		if ( m_pActiveItem && m_pActiveItem->m_iId == WEAPON_GATLING )
			pev->velocity = pev->velocity + (pev->basevelocity / 4.0f);
		else
...
{
	// Shepard : Deny weapon selection if we are holding the gatling
	if ( m_pActiveItem && m_pActiveItem->m_iId == WEAPON_GATLING )
	{
		ClientPrint( pev, HUD_PRINTCENTER, "#Gatling_Drop" );
		return;
	}
...
void CBasePlayer :: FlashlightTurnOn( void )
{
	...
	if ( (pev->weapons & (1 << WEAPON_SUIT)) && m_iFlashBattery > 0 )
	{
		...
		// Shepard : avoid infinite lantern glitch
		m_iFlashBattery--;
...
void CBasePlayer :: ForceClientDllUpdate( void )
{
	m_iClientHealth  = -1;
	m_iClientBattery = -1;
	ForceClientDllUpdateHRP(); // Shepard
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 007:
		...
		switch ( RANDOM_LONG( 0, 3 ) )
		{
		case 0: ALERT( at_console, "You\'re done.\n" ); break;
		case 1: ALERT( at_console, "Really ?\n" ); break;
		case 2: ALERT( at_console, "Are you fucking serious ?\n" ); break;
		case 3: ALERT( at_console, "Winners don\'t use cheats.\n" ); break;
		}
#endif
		if ( m_rgItems[ITEM_COCAINE] < 9 )
			m_rgItems[ITEM_COCAINE]++;

		if ( m_rgItems[ITEM_DRYMEAT] < 9 )
			m_rgItems[ITEM_DRYMEAT]++;

		if ( m_rgItems[ITEM_MEATPIE] < 9 )
			m_rgItems[ITEM_MEATPIE]++;

		if ( m_rgItems[ITEM_WHISKEY] < 9 )
			m_rgItems[ITEM_WHISKEY]++;

		if ( m_rgItems[ITEM_WHALEOIL] < 9 )
			m_rgItems[ITEM_WHALEOIL]++;
...
int CBasePlayer::AddPlayerItem( CBasePlayerItem *pItem )
{
	...
	if (pItem->AddToPlayer( this ))
	{
		...
		// Shepard : If we are holding the gatling gun and we pick up
		// a weapon, then don't switch to it.
		if ( m_pActiveItem && m_pActiveItem->m_iId == WEAPON_GATLING )
			return TRUE;
...
int CBasePlayer::RemovePlayerItem( CBasePlayerItem *pItem )
{
	if (m_pActiveItem == pItem)
	{
		ResetAutoaim( );
		pItem->Holster( );
		pItem->pev->nextthink = 0; // Saber may be trying to swing again, etc.
		pItem->SetThink( NULL );
		m_pActiveItem = NULL;
		pev->viewmodel = 0;
		pev->weaponmodel = 0;
...
void CBasePlayer :: UpdateClientData( void )
{
	if (m_fInitHUD)
	{
		...
		// Shepard : this is required to force the items HUD to be updated every time you load
		// a saved game after launching the game, could also fix some save/restore issues
		m_iClientBuyZone = -1;
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
	...
	if ( m_flFlashLightTime && m_flFlashLightTime <= gpGlobals->time )
	{
		if ( FlashlightIsOn() )
		{
			if ( m_iFlashBattery && !(pev->flags & FL_FROZEN) )
			{
				m_flFlashLightTime = FLASH_DRAIN_TIME + gpGlobals->time;
				m_iFlashBattery--;
...
// the active item. 
//=========================================================
// Shepard : Reworked for Parasomnia's Gatling gun
void CBasePlayer::DropPlayerItem( void )
{
	// Make sure we drop the Gatling
	if ( m_pActiveItem && m_pActiveItem->m_iId != WEAPON_GATLING )
		return;

	// Don't drop if we can't switch to another weapon
	CBasePlayerItem *pGatlingAsItem = m_pActiveItem;
	if ( !g_pGameRules->GetNextBestWeapon( this, pGatlingAsItem ) )
		return;

	// Retrieve the Gatling's ammo type and spawn
	int	iAmmoIndex = GetAmmoIndex( pGatlingAsItem->pszAmmo1() );

	// Spawn a fresh Gatling and assign it the ammo we had
	CBasePlayerWeapon *pGatlingAsWeapon = (CBasePlayerWeapon *)CBaseEntity::Create( "weapon_gatling", pev->origin + gpGlobals->v_forward * 10.0f, pev->angles, edict() );
	pGatlingAsWeapon->pev->spawnflags &= SF_NORESPAWN;
	pGatlingAsWeapon->pev->angles.x = 0.0f;
	pGatlingAsWeapon->pev->angles.z = 0.0f;
	pGatlingAsWeapon->pev->velocity = gpGlobals->v_forward * 300.0f + gpGlobals->v_forward * 100.0f;
	pGatlingAsWeapon->m_iDefaultAmmo = m_rgAmmo[iAmmoIndex];
	m_rgAmmo[iAmmoIndex] = 0;

	// We reset everything to be sure and we remove the previous Gatling from our inventory
	SetSpeed( PLAYER_SPEED_NORMAL );
	UTIL_MakeVectors( pev->angles );
	pev->weapons &= ~(1 << pGatlingAsItem->m_iId);
	pev->gravity = 1.0f;
	pGatlingAsItem->DestroyItem();
}
...
void CStripWeapons :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
	{
		...
		pPlayer->SetSpeed( PLAYER_SPEED_NORMAL ); // Reset the player's speed
	}

	// Shepard : If the "Caleb fight" spawnflag is set, remove all weapons entities (except the saber)
	if ( FBitSet( pev->spawnflags, 1 ) )
	{
		...
		for ( int i = gpGlobals->maxClients + 1; i < gpGlobals->maxEntities; ++i, ++pEdict )
		{
			...
			if ( !pEnt || !strstr( (char *)STRING( pEnt->pev->classname ), "weapon_" ) ||
				strstr( (char *)STRING( pEnt->pev->classname ), "weapon_saber" ) )
				continue;
