...
class CHalfRats : public CTalkMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_halfrats, CHalfRats );

void CHalfRats::Spawn( void )
{
	Precache();
	if ( pev->model )
		SET_MODEL( ENT( pev ), STRING( pev->model ) ); // LRC
	else
		SET_MODEL( ENT( pev ), "models/halfrats.mdl" );
	...
	MonsterInit();
	SetUse( &CHalfRats::FollowerUse );
}

void CHalfRats::Precache( void )
{
	if ( pev->model ) // LRC
		PRECACHE_MODEL( (char *)STRING( pev->model ) );
	else
		PRECACHE_MODEL( "models/halfrats.mdl" );

	// Every new Half-Rats must call this, otherwise when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	CTalkMonster::Precache();
}

void CHalfRats::SetYawSpeed( void )
{
	int iYawSpeed = 90;
	switch ( m_Activity )
	{
...
int CHalfRats::Classify( void )
{
	return CLASS_HUMAN_PASSIVE;
}

void CHalfRats::Killed( entvars_t *pevAttacker, int iGib )
{
	SetUse( NULL );
	CTalkMonster::Killed( pevAttacker, iGib );
}

void CHalfRats::TalkInit( void )
{
