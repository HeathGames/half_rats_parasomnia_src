...
void CFade::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	CBasePlayer *pPlayer = NULL; // Shepard : To deny gameplay fades
	if ( pev->spawnflags & SF_FADE_ONLYONE )
	{
		if ( pActivator->IsNetClient() )
		{
			pPlayer = (CBasePlayer *)pActivator; // Shepard : Deny gameplay fades
			if ( pPlayer )
			{
				pPlayer->m_flCinematicFadeTime = gpGlobals->time + HoldTime();
	...
	{
		for ( int i = 1; i <= gpGlobals->maxClients; i++ ) // Shepard : Deny gameplay fades
		{
