...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_MONSTER_COLT:
		case BULLET_PLAYER_HENRY:
		case BULLET_PLAYER_BUCKSHOT:
		case BULLET_PLAYER_GATLING:
		case BULLET_MONSTER_HENRY:
		default:
			// smoke and decal
...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "ai_marker" );
#ifdef DEBUG
	UTIL_PrecacheOther( "item_cocaine" );
	UTIL_PrecacheOther( "item_drymeat" );
	UTIL_PrecacheOther( "item_meatpie" );
	UTIL_PrecacheOther( "item_money" );
	UTIL_PrecacheOther( "item_whaleoil" );
	UTIL_PrecacheOther( "item_whiskey" );

	UTIL_PrecacheOther( "monster_armorman" );
	UTIL_PrecacheOther( "monster_baph" );
	UTIL_PrecacheOther( "monster_barnaby" );
	UTIL_PrecacheOther( "monster_bear" );
	UTIL_PrecacheOther( "monster_bernard" );
	UTIL_PrecacheOther( "monster_caleb" );
	UTIL_PrecacheOther( "monster_clown" );
	UTIL_PrecacheOther( "monster_culthermit" );
	UTIL_PrecacheOther( "monster_dog" );
	UTIL_PrecacheOther( "monster_doktor" );
	UTIL_PrecacheOther( "monster_duck" );
	UTIL_PrecacheOther( "monster_halfcat" );
	UTIL_PrecacheOther( "monster_halfrats" );
	UTIL_PrecacheOther( "monster_huntsman" );
	UTIL_PrecacheOther( "monster_man" );
	UTIL_PrecacheOther( "monster_maskboy" );
	UTIL_PrecacheOther( "monster_pig" );
	UTIL_PrecacheOther( "monster_publican" );
	UTIL_PrecacheOther( "monster_teddy" );
	UTIL_PrecacheOther( "monster_trask" );
	UTIL_PrecacheOther( "monster_traskboss" );
	UTIL_PrecacheOther( "monster_udcop" );
	UTIL_PrecacheOther( "monster_upperclassman" );
	UTIL_PrecacheOther( "monster_woman" );
	...
	UTIL_PrecacheOther( "ammo_arrowbox" );
#endif
...
#ifdef USE_MP_WEAPONS
	// Half-Cat
	UTIL_PrecacheOtherWeapon( "weapon_halfcat" );
#endif
	...
	PRECACHE_SOUND( "weapons/tail1.wav" );
	PRECACHE_SOUND( "weapons/tail2.wav" );
	PRECACHE_SOUND( "weapons/tail3.wav" );
	PRECACHE_SOUND( "weapons/tail4.wav" );
}
...
void CBasePlayerWeapon::ItemPostFrame( void )
{
	...
	{
		// reload when reload is pressed, or if no buttons are down.
		Reload();
	}
...
BOOL CBasePlayerWeapon :: PlayEmptySound( void )
{
	...
	{
		// Shepard : custom dry fire sound
		switch ( m_iId )
		{
		case WEAPON_DEBUG: EMIT_SOUND( ENT( m_pPlayer->pev ), CHAN_WEAPON, "weapons/colt_cock.wav", 0.8f, ATTN_NORM ); break;
		case WEAPON_COLT: EMIT_SOUND( ENT( m_pPlayer->pev ), CHAN_WEAPON, "weapons/colt_cock.wav", 0.8f, ATTN_NORM ); break;
		case WEAPON_HENRY: EMIT_SOUND( ENT( m_pPlayer->pev ), CHAN_WEAPON, "weapons/henry_cock.wav", 0.8f, ATTN_NORM ); break;
		case WEAPON_SHOTGUN: EMIT_SOUND( ENT( m_pPlayer->pev ), CHAN_WEAPON, "weapons/shotgun_cock.wav", 0.8f, ATTN_NORM ); break;
#ifdef USE_MP_WEAPONS
		case WEAPON_BOW: break;
#endif
		case WEAPON_GATLING: EMIT_SOUND( ENT( m_pPlayer->pev ), CHAN_WEAPON, "weapons/gatling_cock.wav", 0.8f, ATTN_NORM ); break;
		default: EMIT_SOUND( ENT( m_pPlayer->pev ), CHAN_WEAPON, "weapons/357_cock1.wav", 0.8f, ATTN_NORM );
		}
		m_iPlayEmptySound = 0;
		return 0;
	}
...
{
	// Shepard : If we are holding the gatling gun and we are trying to pick up another gatling gun
	// then don't add the ammo. This could cause a bug where the player could empty his Gatling gun,
	// drop it, pick it up again and the change weapon code wouldn't be triggered. Then the player
	// could carry a fully loaded Gatling gun by picking another one.
	if ( pWeapon->m_pPlayer->m_pActiveItem && pWeapon->m_pPlayer->m_pActiveItem->m_iId == WEAPON_GATLING && pWeapon->m_iId == WEAPON_GATLING )
		return FALSE;
	...
	if ( pszAmmo1() != NULL )
	{
		...
		// Shepard : The BUGFIX in 1.0 about updating the barrels when picking up a new shotgun if empty
		// would make player confused, so here's a better solution
		if ( pWeapon->m_iId == WEAPON_SHOTGUN )
		{
			BOOL bFound = FALSE;
			CBasePlayerItem *pPlayerItem = NULL;
			int i = 0;

			// Find the shotgun in the backpack
			while ( i < MAX_ITEM_TYPES && !bFound )
			{
				pPlayerItem = pWeapon->m_pPlayer->m_rgpPlayerItems[i];
				while ( pPlayerItem && !bFound )
				{
					if ( pPlayerItem->m_iId == WEAPON_SHOTGUN ) // There it is
						bFound = TRUE;
					else
			...
			if ( bFound ) // We've found the shotgun, update it
			{
				CShotgun *pShotgun = (CShotgun *)pPlayerItem;
				if ( pShotgun )
				{
					if ( m_iDefaultAmmo == 2 )
					{
						pShotgun->m_bIsLeftBarrelEmpty = false;
						pShotgun->m_bIsRightBarrelEmpty = false;
						pShotgun->m_iParaExtraStuff = 0;
					}
					else if ( m_iDefaultAmmo == 1 )
					{
						pShotgun->m_bIsLeftBarrelEmpty = false;
						pShotgun->m_bIsRightBarrelEmpty = true;
						pShotgun->m_iParaExtraStuff = 2;
					}
					else
					{
						pShotgun->m_bIsLeftBarrelEmpty = true;
						pShotgun->m_bIsRightBarrelEmpty = true;
						pShotgun->m_iParaExtraStuff = 3;
					}
				}
				else
					ALERT( at_console, "[HR:P] Failed to get pointer to CShotgun! Barrels won\'t be updated!\n" );
			}
		}
...
void CWeaponBox::Precache( void )
{
	...
	// Shepard : Precache the gatling gun model (because it can be dropped)
...
BOOL CWeaponBox::PackWeapon( CBasePlayerItem *pWeapon )
{
	...
	pWeapon->SetThink( NULL ); // Saber may be trying to swing again, etc.
...
TYPEDESCRIPTION CDebug::m_SaveData[] =
{
	DEFINE_FIELD( CDebug, m_iMode, FIELD_INTEGER )
};
IMPLEMENT_SAVERESTORE( CDebug, CBasePlayerWeapon );
