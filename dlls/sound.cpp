...
void EMIT_SOUND_SUIT(edict_t *entity, const char *sample)
{
	...
	if (fvol > 0.05)
		EMIT_SOUND_DYN(entity, CHAN_STATIC, sample, fvol, ATTN_NORM, 0, 100);
}
...
void EMIT_GROUPID_SUIT(edict_t *entity, int isentenceg)
{
	float fvol = CVAR_GET_FLOAT("suitvolume");
	if (fvol > 0.05)
		SENTENCEG_PlayRndI(entity, isentenceg, fvol, ATTN_NORM, 0, 100);
}
...
void EMIT_GROUPNAME_SUIT(edict_t *entity, const char *groupname)
{
	float fvol = CVAR_GET_FLOAT("suitvolume");
	if (fvol > 0.05)
		SENTENCEG_PlayRndSz(entity, groupname, fvol, ATTN_NORM, 0, 100);
}
