...
// override some values for multiplay.
	...
	// Cavalry saber whack
	gSkillData.plrDmgSaber = 25;

	// Colt SAA Round
	gSkillData.plrDmgColt = 20;

	// Henry rifle Round
	gSkillData.plrDmgHenry = 40;

	// Double barrel shotgun buckshot
	gSkillData.plrDmgBuckshot = 6;

#ifdef USE_MP_WEAPONS
	// Bow Arrow
	gSkillData.plrDmgBowArrow = 75;
#endif

	// Gatling Round
	gSkillData.plrDmgGatling = 10;

	// Dynamite
	gSkillData.plrDmgDynamite = 100;
}
...
BOOL CHalfLifeMultiplay :: GetNextBestWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pCurrentWeapon )
{
	...
	// at least get the saber, but ya never know.
	if ( !pBest )
	{
		return FALSE;
	}
...
void CHalfLifeMultiplay :: PlayerSpawn( CBasePlayer *pPlayer )
{
	...
	if ( addDefault )
	{
		pPlayer->GiveNamedItem( "weapon_saber" );
		pPlayer->GiveNamedItem( "weapon_colt" );
		pPlayer->GiveAmmo( 24, "45", _45_MAX_CARRY ); // 4 full reloads
	}
}
...
{
	// Shepard : The Gatling gun should take more time to respawn
	BOOL bIsGatling = FClassnameIs( pWeapon->pev, "weapon_gatling" );

	if ( weaponstay.value > 0 && !bIsGatling )
	{
	...
	if ( bIsGatling )
		return gpGlobals->time + (WEAPON_RESPAWN_TIME * 3.0f);
	else
...
{
	// Shepard : The cocaine should take more time to respawn
	if ( FClassnameIs( pItem->pev , "item_cocaine" ) )
		return gpGlobals->time + (ITEM_RESPAWN_TIME * 2.0f);
	else
