...
//			m_iShards = atof(pkvd->szValue);
	...
	else if (FStrEq(pkvd->szKeyName, "explodemagnitude") )
	{
		...
		pkvd->fHandled = TRUE;
	else if ( FStrEq( pkvd->szKeyName, "obj_amount" ) ) // Shepard
	{
		m_iObjectAmount = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if ( FStrEq( pkvd->szKeyName, "obj_body" ) ) // Shepard
	{
		m_iObjectBody = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
...
//	DEFINE_FIELD( CBreakable, m_idShard, FIELD_INTEGER ),
	...
	DEFINE_FIELD( CBreakable, m_iObjectAmount, FIELD_INTEGER ), // Shepard
	DEFINE_FIELD( CBreakable, m_iObjectBody, FIELD_INTEGER ) // Shepard
...
void CBreakable::Spawn( void )
{
	...
	// Shepard : Spawn X times the entity
	if ( m_iObjectAmount < 1 )
		m_iObjectAmount = 1;
}
...
//		return;
	...
	if (i)
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, rgpsz[RANDOM_LONG(0,i-1)], fvol, ATTN_NORM, 0, PITCH_NORM);
}
...
int CBreakable :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( pevAttacker == pevInflictor )	
	{
		...
		// if a client hit the breakable with a saber, and breakable is saber-sensitive, break it now.
		if ( FBitSet ( pevAttacker->flags, FL_CLIENT ) &&
				 FBitSet ( pev->spawnflags, SF_BREAK_SABER ) && (bitsDamageType & DMG_SLASH))
			flDamage = pev->health;
	}
	else
	...
	// Breakables take double damage from the saber
...
void CBreakable::Die( void )
{
	...
	case matGlass:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustglass1.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustglass2.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		}
		cFlag = BREAK_GLASS;
		break;
	...
	case matWood:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustcrate1.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustcrate2.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		}
		cFlag = BREAK_WOOD;
		break;
	...
	case matMetal:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustmetal1.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustmetal2.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		}
		cFlag = BREAK_METAL;
		break;
	...
	case matFlesh:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustflesh1.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustflesh2.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		}
		cFlag = BREAK_FLESH;
		break;
	...
	case matCinderBlock:
		switch ( RANDOM_LONG(0,1) )
		{
		case 0:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustconcrete1.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		case 1:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustconcrete2.wav", fvol, ATTN_NORM, 0, PITCH_NORM);	
			break;
		}
	...
	case matCeilingTile:
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustceiling.wav", fvol, ATTN_NORM, 0, PITCH_NORM);
		break;
	}
	...
	if ( m_iszSpawnObject )
	{
		CBaseEntity *pSpawnedEntity;
		if ( m_iObjectAmount > 1 ) // Shepard : Spawn multiple times the same entity
		{
			for ( int i = 0; i < m_iObjectAmount; i++ )
			{
				pSpawnedEntity = CBaseEntity::Create( (char *)STRING( m_iszSpawnObject ), VecBModelOrigin( pev ) + Vector( 0.0f, 0.0f, 4.0f * i ), pev->angles, edict() );
				if ( pSpawnedEntity )
					pSpawnedEntity->pev->body = m_iObjectBody;
			}
		}
		else
		{
			pSpawnedEntity = CBaseEntity::Create( (char *)STRING( m_iszSpawnObject ), VecBModelOrigin( pev ), pev->angles, edict() );
			if ( pSpawnedEntity ) // Shepard : Spawn the entity with a specific body
				pSpawnedEntity->pev->body = m_iObjectBody;
		}
	}
